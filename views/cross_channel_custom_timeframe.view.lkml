...
# =B3-$E$2-($C$2-1)
  dimension: date_transformed {
    type: date
    sql: {% condition timeframe_b %} DATE_ADD(${date_raw} , INTERVAL ${total_alignment_needed} DAY) {% endcondition %} END  ;;
  }
...
### Build Out the Report
  ...
  measure: kpi_banner {
    ...
    html:
      <div class="vis" >
          <div class="vis-single-value" style="display:flex; align-items:flex-start width=100% font-size:30px; background-image: linear-gradient(to right, #1b1662, #000000, #91aa2d); color:#ffffff">
            <div style="flex: 1;"><b>MEDIA THAT MOVES WITH YOU</b></div><div style="width: 60px">dentsu | PELOTON</div>
       </div>
      </div>;;
  }
